 ...
 * @author jungsun
 */
public class Panel4AlarmHistoryFilter extends Panel {
	 ...
	 * @author Hello NMS
	 */
	public interface Panel4AlarmHistoryFilterListenerIf {
	...
	protected Panel4AlarmHistoryFilterListenerIf listener;
	...
	protected SelectorCombo selectorComboAlarmCode;
	 ...
	 *            리스너
	 */
	public Panel4AlarmHistoryFilter(Composite parent, int style, Panel4AlarmHistoryFilterListenerIf listener) {
		super(parent, style, UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.FILTER_TITLE, MESSAGE_CODE_ORANGE.ALARM_HISTORY));
	...
	protected void createGUI() {
		...
		selectorCalendarFromDate.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				listener.queryListAlarmHistory(0);
			}
		});
		...
		labelSeverity.setText(UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.ALARM_SEVERITY));

		selectorComboAlarmSeverity = new SelectorCombo(panelContents.getContentComposite(), SWT.READ_ONLY);
		selectorComboAlarmSeverity.setDataCombo(DataFactory.createDataCombo(DATA_COMBO_ORANGE.ALARM_HISTORY_SEVERITY));
		...
		LabelText labelAlarmCode = new LabelText(panelContents.getContentComposite(), SWT.NONE);
		labelAlarmCode.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		labelAlarmCode.setText(UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.ALARM_CODE));

		selectorComboAlarmCode = new SelectorCombo(panelContents.getContentComposite(), SWT.READ_ONLY);
		selectorComboAlarmCode.setDataCombo(DataFactory.createDataCombo(DATA_COMBO_ORANGE.ALARM_HISTORY_CODE));
		selectorComboAlarmCode.addSelectionChangedListener(new ISelectionChangedListener() {
			@Override
			public void selectionChanged(SelectionChangedEvent event) {
				listener.queryListAlarmHistory(0);
			}
		});
		selectorComboAlarmCode.getControl().setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
	}
	...
	public EMP_MODEL_EVENT getEventCode() {
		Object selectedItem = selectorComboAlarmCode.getSelectedItem();
	...
	public void setEventCode(EMP_MODEL_EVENT event_def) {
		selectorComboAlarmCode.setSelectedItem(event_def);
	}
