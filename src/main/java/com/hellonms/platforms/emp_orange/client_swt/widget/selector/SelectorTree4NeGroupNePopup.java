...
public class SelectorTree4NeGroupNePopup extends PanelPopup {
	 ...
	 * NE 셀렉터의 트리모델
	 */
	protected DataTree4NetworkTree dataTree;
	 ...
	 *            관리여부
	 */
	public SelectorTree4NeGroupNePopup(Shell shell, NODE node, int[] ne_group_id_filters, int[] ne_id_filters) {
		super(shell);
		this.node = node;
		dataTree = (DataTree4NetworkTree) DataFactory.createDataTree(DATA_TREE_ORANGE.NETWORK_TREE);
	...
	protected void createGUI() {
		...
		panelTree.getTree().addMouseListener(new MouseAdapter() {
			...
			public void mouseDoubleClick(MouseEvent e) {
				SelectorTree4NeGroupNePopup.this.performFinish();
			}
		});
		...
		buttonCancel.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				SelectorTree4NeGroupNePopup.this.performCancel();
			}
		});
