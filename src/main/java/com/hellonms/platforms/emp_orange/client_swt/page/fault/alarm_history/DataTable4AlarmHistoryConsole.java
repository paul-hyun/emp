 ...
 * @author jungsun
 */
public class DataTable4AlarmHistoryConsole extends DataTableAt {
	 ...
	 * 컬럼 너비 배열
	 */
	protected int[] COLUMN_WIDTHS = { 60, 100, 135, 135, 120, 190, 190, 120, 320, 320 };
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case 3:
			return alarm.isClear_state() ? UtilDate.format(alarm.getClear_time()) : "";
		case 4:
			try {
		...
		case 9:
			return alarm.getClear_description();
		default:
	...
	public Color getBackground(Object element) {
		Model4Alarm alarm = (Model4Alarm) element;
		if (alarm.isClear_state()) {
			return UtilResource.getColor(new int[] { 71, 103, 71 });
		} else {
			return null;
		}
	}
	...
	public void clear() {
		...
		try {
			for (Model4Alarm alarm : allAlarmList) {
				this.maxEventId = Math.max(this.maxEventId, alarm.getClear_event_id());
			}
		} finally {
			lock.unlock();
		}
	 ...
	 *            이벤트 모델 배열
	 */
	protected void setData(Model4Alarm[] model4AlarmHistories) {
		lock.lock();
		try {
			ArrayList<Model4Alarm> alarmNoList = new ArrayList<Model4Alarm>();
			for (int i = 0; i < model4AlarmHistories.length; i++) {
				alarmNoList.add(model4AlarmHistories[i]);
			}
			this.allAlarmList = alarmNoList;
		} finally {
			lock.unlock();
		}
	}
	 ...
	 * @return 표시상태
	 */
	protected boolean isDisplay(Model4Alarm model4Alarm) {
		boolean display = !model4Alarm.isClear_state() || maxEventId < model4Alarm.getClear_event_id();

		if (display) {
			SEVERITY severity = model4Alarm.getSeverity();
		...
		if (display && neGroupFilters != null) {
			...
			for (Model4NeGroup neGroupFilter : neGroupFilters) {
				display_ne_group = ModelClient4NetworkTree.getInstance().isChildNe(neGroupFilter.getNe_group_id(), model4Alarm.getNe_id());
				if (display_ne_group) {
					break;
				}
			}
