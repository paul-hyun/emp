 ...
 * @author jungsun
 */
public class Wizard4DeleteNeInfo extends Wizard {
	 ...
	 * @author Hello NMS
	 */
	public interface Wizard4DeleteNeInfoListenerIf {
		 ...
		 *            사용자 모델
		 */
		public void deleteNeInfo(Wizard4DeleteNeInfo wizard, Model4NeInfoIf user);
	 ...
	 *            리스너
	 */
	public static void open(Shell shell, Model4NeInfoIf model4NeInfo, Wizard4DeleteNeInfoListenerIf listener) {
		if (instance == null || instance.getShell() == null || instance.getShell().isDisposed()) {
			instance = new DialogWizard(shell, new Wizard4DeleteNeInfo(model4NeInfo, listener));
			instance.open();
		}
	}
	...
	private Wizard4DeleteNeInfoListenerIf listener;

	private Wizard4DeleteNeInfo(Model4NeInfoIf model4NeInfo, Wizard4DeleteNeInfoListenerIf listener) {
		setWindowTitle(UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.DELETE_TITLE, model4NeInfo.getNe_info_def().getName()));
	...
	public void addPages() {
		wizardPage4NeInfo = new WizardPage4NeInfo(PANEL_INPUT_TYPE.DELETE, model4NeInfo);
	...
	public boolean performFinish() {
		...
		if (DialogMessage.openConfirm(getShell(), UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.DELETE_TITLE, model4NeInfo.getNe_info_def().getName()), UtilLanguage.getMessage(MESSAGE_CODE_ORANGE.DELETE_CONFIRM, MESSAGE_CODE_ORANGE.NE_INFO, model4NeInfo.getNe_info_def().getName()))) {
			listener.deleteNeInfo(this, model4NeInfo);
		}
		return false;
	}
